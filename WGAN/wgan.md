...
+ 기존 GAN의 경우 Generator와 Discimininator가 서로 균형을 이루면서 학습이 진행되어야만 하지만, WGAN의 경우 critic을 최대한으로 최적점까지 학습시키는 것을 통해서 generator에게 더 좋은 gradient를 제공한다는 점에서 학습의 차이가 있다.
...
+ 모델을 사용하는 입장에서 기억해야 할 점이라고 한다면
  - loss가 이미지의 quality와 correlation을 가지고 있기 때문에 loss 값을 보고서 학습이 잘 진행되고 있는지 파악하기가 쉽다는 점이 있다.    
  - 논문의 그림에 나온 대로, 점점 loss가 줄어들고 이에 따라서 이미지의 quality가 좋아지는 것을 확인하면 학습이 잘 이루어지고 있다는 것을 파악할 수 있다.   

+ 다른 논문들의 문제정의와 문제해결방식과는 다르게 정말 통계적 이론을 깊게 파고 들어 여러 case들의 제안과 증명까지.. 등등 deep하게 연구했다는 점이 정말 대단하게 느꼈다.
